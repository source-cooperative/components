...
export default function SearchBar(props: SearchBarProps) {
    ...
    return <Box as="form" onSubmit={handler} sx={{width: "100%"}}>
        <Grid sx={{gridGap: 3, gridTemplateColumns: "auto max-content"}}>
            <Input placeholder={placeholder} name="query" defaultValue={defaultValue}/>
            <Button variant="nav" onClick={handler}>{buttonText}</Button>
        </Grid>
    </Box>
}
