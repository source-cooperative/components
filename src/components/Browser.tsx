...
import { Paragraph, Grid, Flex, Box, Card, Text } from "theme-ui";
...
export default function Browser(props: BrowserResult) {
    ...
    return (
        ...
        <Box sx={{py: 2}}>
            ...
            {
                !breadcrumbs || breadcrumbs.length == 0 ? <Skeleton />
                : <>
                    {
                        breadcrumbs?.map((breadcrumb, i) => {
                            return <Box key={`breadcrumb-${i}`} sx={{display: "inline"}}><Link variant="breadcrumb" href={breadcrumb.href}>{breadcrumb.name}</Link><Text>/</Text></Box>
                        })
                    }
                </>
            }
        ...
        <Card variant="code" sx={{fontSize: 1, fontFamily: "mono"}}>
            {
                !items ? Array(10).fill(1).map((x, y) => {
                    return <Box sx={{width: "100%"}}><Skeleton /></Box>
                }) : <></>
            }
            {
                items?.map((item, i) => {
        ...
        >
            <Box sx={{textAlign: "left"}}>
                <Button onClick={() => router.back()} disabled={!prev}>Prev</Button>
            </Box>
            <Box sx={{textAlign: "right"}}>
                <Button disabled={!items || !next} href={next}>Next</Button>
            </Box>
